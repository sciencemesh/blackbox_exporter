...
var (
	...
	Probers = map[string]prober.ProbeFn{
		...
		"dns":    prober.ProbeDNS,
		"nagios": prober.ProbeNagios, // SM: Register the custom Nagios prober
	}
)
...
func run() int {
	...
	kingpin.Version(version.Print("blackbox_exporter [ScienceMesh edition]"))
	kingpin.HelpFlag.Short('h')
	kingpin.Parse()
	logger := promlog.New(promlogConfig)
	rh := &resultHistory{maxResults: *historyLimit}

	level.Info(logger).Log("msg", "Starting blackbox_exporter [ScienceMesh edition]", "version", version.Info())
